...
class Train():
    ...
    def __init__(self, transitionmemory: Buffer, MODELDIRECTORY: str, LOADMODEL = False, lr = 0.00005, membatch_size = 100, scheduler_stepsize = 50, gamma = 0.99, modeltype = "CNN"):
        ...
        if self.modeltype == "CNN":
            self.simulator = SimulatorCNN()
            self.simulator = self.simulator.cuda()
        elif self.modeltype == "GAN":
            self.simulator = SimulatorGAN()
            self.simulator = self.simulator.cuda()

        self.optimizer = optim.AdamW(self.simulator.parameters(), self.lr)
        self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=scheduler_stepsize, gamma=self.gamma)
        ...
        run = wandb.init(project = 'CC_OPM_Simulator', entity = 'elijahfrench2023',  config = {"LR": self.lr, "Batch Size": self.membatch_size, "Scheduler Stepsize": self.scheduler_stepsize, "ModelType": self.modeltype},
    ...
    def learn(self):
        ...
        simulated = self.simulator(state_action)
        state_s = state_s.cuda()

        loss = self.L(state_s, simulated)

        wandb.log({"Simulator": float(loss)})
    ...
    def grids(self, state, state_, action, wandb_plot = True, pngname = ''):
        ...
        simulated = self.simulator(state_action).cpu()
